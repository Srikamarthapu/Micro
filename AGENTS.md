...
- The durable visual source of truth is `/Users/kamarthapusri/.codex/attachments/532c6efa-9a59-4b96-a512-abf06067913b/image-1.png` (the user's original selected direction). Keep its parchment warmth, quiet watercolor map, deep navy typography, teal primary actions, blue Community Help accents, and restrained purple Sponsored accents.
- Do not replace this direction with the later brighter, rainbow-like field-guide exploration. The user explicitly reversed that choice before implementation.
- Preserve the original screen's calm density and civic warmth: map plus nearby-task sheet, rounded cream surfaces, large readable labels, specific trust copy, and five persistent labeled tabs.
