...
- Micro has two production account types: regular neighbors and nonprofit organizations. Both may post and accept tasks. Keep this account-type model separate from the adult/youth/guardian prototype personas, and grant sponsorship-request or sponsor-task capabilities only from active database organization membership plus verified organization state—not signup metadata or client-side role toggles.
...
- Requesters do not write listings on the normal path. Every published task comes from the reviewed catalog in `src/taskCatalog.ts` (200 tasks across 12 categories): the requester picks an entry, answers its bounded options, and `composeListing()` writes the title, description, inclusions, exclusions, completion check, duration, and suggested pay. Moderation is a property of the data rather than a filter applied afterwards. Do not add free-text title, description, scope, or completion fields to the catalog path; if a task is missing, add a catalog entry with its own safety boundary and pay band. The private match address is the one typed field on that path and is never public.
- The single exception is a custom task, offered **only** when a search returns no match — never alongside results, and never as a general shortcut. It types a title and a description and nothing else: category, duration, and the completion check stay bounded, and the safety boundary plus baseline exclusions are inherited from the chosen category. `customTextProblem()` rejects prohibited work in that text, `buildCustomTemplate()` shapes it into an ordinary `TaskTemplate`, and `isCustom` follows it onto the listing as `Task.customPending`. A custom task never earns youth eligibility, and every surface that shows one must keep saying it is awaiting review — do not quietly drop that marker to tidy a card.
- The Post tab opens directly on the catalog — search over all 200 tasks, a most-requested shortlist, and a grid showing all 12 categories at once. Do not hide categories behind a horizontal rail, and do not put an abstract question ahead of the concrete one: people know what they need done before they know how they want it arranged. Drilling into a category surfaces its safety boundary before any task is picked. Because the task comes first, the arrangement step offers only `template.modes`, so a volunteer-only task can never be posted as paid; when a task change makes the held mode invalid, fall back to `modes[0]`.
- The posting flow's "when" options are derived from the clock, never hardcoded: `dateChoicesFor()` offers Today, Tomorrow, a real weekday two days out, and Flexible. Today is dropped once fewer than `sameDayLeadMinutes` (60) remain before the last slot, and choosing Today trims the start-time rail to slots still that far out, snapping a stale selection forward. Do not reintroduce a fixed weekday string like "Saturday" — it collides with Tomorrow one day a week and lets a requester post a start time in the past.
...
- Micro's primary geography is Oakland and Alameda, with the collaborator-added Montréal preview region retained. Areas are a bounded enum in `src/Prototype.tsx` (`areas`): `all` (Oakland & Alameda), `downtown`, `temescal`, `fruitvale`, `westoak`, `alameda`, and `montreal`. Each carries a `center`, a `zoom`, and a display label. Do not reintroduce free-text area strings, and keep `all` as the explicit Oakland/Alameda region-wide sentinel rather than overloading a real neighborhood.
- Every task carries required `coords: LatLng` and an `areaId`. Distance is always computed with `distanceMiles()` from the viewer's own profile area, never stored as a string.
...
- Be explicit about the service boundary. Supabase authentication, profile/account type, organization membership, and database-derived sponsorship capabilities are live. Google may render the basemap when configured. Task records and markers, approximate locations, photos, youth/guardian identity fixtures, messages, payments, moderation, and notifications remain browser-local preview state until secure backend services exist.
